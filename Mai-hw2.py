...
def id3(examples, target, attributes):
    ...
    # Case 3: else, build the tree recursively
        ...
        for val in vals:
            newAttr = attributes[:]
            new_examples = split_on_val(examples, newAttr, best_attribute_label, val, target)
            # newAttr was added target column in the split_on_val so we remove it now
            newAttr.remove(target)    
...
def entropy(examples, target):
    N = len(examples)
    # Count all the unique values in of the target and their occurences
...
def split_on_val(examples, attributes, best, val, target):
    ...
    for row in examples.itertuples(index=False, name = 'Panda') :
        #If meet a row with val in it, we gonna extract that row
        if (row[i] == val):
            new_row = []
            # Make sure only copy column that not best attribute
...
def info_gain(examples, attributes, attr, target):
    ...
    for key in counter:
        ...
        header = attributes[:]
        new_entropy += prob * entropy(split_on_val(examples, header, attr, key, target)\
                    , target)
